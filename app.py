...
from configs import MQTT_IP, MQTT_PORT, HOST, PORT
...
# 節點風扇狀態改變
def fan_state_change(state: State, var_name: str, value):
...
# 節點空調狀態改變
def AC_status_change(state: State, var_name: str, value):
...
# 節點資料更新
def data_update(state: State, topic: str, data: pd.DataFrame):
    ...
    if topic == "2706/IAQ/2":
        ...
        state.first_meeting_room_data["Temperature"] = list(
            data["Temperature"].iloc[-60:]
        )
        ...
        state.second_meeting_room_data["Temperature"] = list(
            data["Temperature"].iloc[-60:]
        )
        ...
        ac_status = pd.read_csv("csv/2706-Air_Condiction-A-status.csv")["Status"]
        state.AC_status = "ON" if ac_status.iloc[-1] == "On" else "OFF"
        ...
        state.engine_room_data["ACSwitchLog"] = list(
            ac_status.iloc[-60:].apply(lambda x: int(x == "On"))
        )
...
def get_data(gui: Gui, state_id_list: list):
    ...
    invoke_callback(
        gui, state_id_list[0], data_update, ["DL303/Info", pd.read_csv("csv/DL303.csv")]
    )
    invoke_callback(
        gui,
        state_id_list[0],
...
def scheduled_tasks(loc: str):
    if loc == "2706":
        client.publish(
            "2706/IAQ/1/control", json.dumps({"fan_0": "OFF", "fan_1": "OFF"})
        )
        client.publish(
            "2706/IAQ/2/control", json.dumps({"fan_0": "OFF", "fan_1": "OFF"})
        )
    elif loc == "Meeting":
...
def conditional_judgment():
    if (
        front_door_data["CO2"][-1] > co2_upper_limit
        or back_door_data["CO2"][-1] > co2_upper_limit
    ):
        if datetime.now().hour >= 10 and datetime.now().hour < turn_off_fan_time:
            if (
                pd.read_csv("csv/2706-IAQ-2.csv")["fan_0"].iloc[-1] == "OFF"
                or pd.read_csv("csv/2706-IAQ-2.csv")["fan_1"].iloc[-1] == "OFF"
            ):
                client.publish(
                    "2706/IAQ/2/control", json.dumps({"fan_0": "IN", "fan_1": "IN"})
                )
            if (
                pd.read_csv("csv/2706-IAQ-1.csv")["fan_0"].iloc[-1] == "OFF"
                or pd.read_csv("csv/2706-IAQ-1.csv")["fan_1"].iloc[-1] == "OFF"
            ):
                client.publish(
                    "2706/IAQ/1/control", json.dumps({"fan_0": "OUT", "fan_1": "OUT"})
                )
    ...
    if engine_room_data["Temperature"][-1] > tc_upper_limit:
        if datetime.now().hour >= 10 and datetime.now().hour < turn_off_ac_time:
            if (
                pd.read_csv("csv/2706-Air_Condiction-A-status.csv")["Status"].iloc[-1]
                == "Off"
            ):
                client.publish(
                    "2706/Air_Condiction/A/control", json.dumps({"Status": "ON"})
                )
        ...
        if (
            ...
            client.publish(
                "2706/Air_Condiction/A/control", json.dumps({"Status": "OFF"})
            )
